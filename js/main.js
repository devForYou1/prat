...
function initializeApp(contentData) {
    ...
    // Content Section for main title, intro paragraphs, motto, and signature
    const contentSection = document.createElement('div');
    contentSection.className = 'position-relative z-10 text-primary-dark';
    contentSection.style.color = '#0A4A7A';

    const header = document.createElement('header');
    header.className = 'text-center';

    const mainTitle = document.createElement('h1');
    mainTitle.className = 'fs-2 fs-md-1 fw-bolder mb-4 lh-sm main-title';
    mainTitle.style.color = '#0A4A7A';
    mainTitle.textContent = contentData.mainTitle;
    header.appendChild(mainTitle);

    const introParagraphsDiv = document.createElement('div');
    introParagraphsDiv.className = 'mw-100 mx-auto fs-6 mb-4 intro-paragraphs';
    introParagraphsDiv.style.color = '#0A4A7A';
    contentData.introParagraphs.forEach(pText => {
        const p = document.createElement('p');
        p.className = 'mb-3';
        p.textContent = pText;
        introParagraphsDiv.appendChild(p);
    });
    header.appendChild(introParagraphsDiv);

    const motto = document.createElement('p');
    motto.className = 'fs-3 fs-md-2 fw-bolder mb-4 text-transparent bg-clip-text motto';
    motto.style.cssText = 'background-image: linear-gradient(to right, #50B8EE, #0A4A7A); -webkit-background-clip: text; -webkit-text-fill-color: transparent;';
    motto.textContent = contentData.motto;
    header.appendChild(motto);

    const signatureDiv = document.createElement('div');
    contentData.signature.forEach(lineText => {
        const p = document.createElement('p');
    ...
    header.appendChild(signatureDiv);
    ...
    searchInput.placeholder = 'חיפוש מידע...';
    searchInput.setAttribute('aria-label', 'חיפוש מידע');

    const searchIcon = createSvgElement('svg', {
        xmlns: "http://www.w3.org/2000/svg",
        viewBox: "0 0 24 24",
        fill: "currentColor",
        class: "w-6 h-6 search-icon"
    });
    searchIcon.innerHTML = `<path fill-rule="evenodd" d="M10.5 3.75a6.75 6.75 0 100 13.5 6.75 6.75 0 000-13.5zM2.25 10.5a8.25 8.25 0 1114.59 5.28l4.69 4.69a.75.75 0 11-1.06 1.06l-4.69-4.69A8.25 8.25 0 012.25 10.5z" clip-rule="evenodd" />`;
    ...
    // Accordion Sections
    const accordionSectionsDiv = document.createElement('div');
    accordionSectionsDiv.className = 'position-relative z-10 mt-4';
    contentData.sections.forEach(section => {
        const subItemElements = [];
        section.subItems.forEach(item => {
            subItemElements.push(SubCard(item.title, () => openModal(item.content), item.id));
        });
        accordionSectionsDiv.appendChild(AccordionItem(section.title, section.summary, subItemElements, section.id));
    });
    mainContainer.appendChild(accordionSectionsDiv);

    // Footer Card with external links
    const footerCard = document.createElement('div');
    footerCard.className = 'footer-card';

    const footerTitle = document.createElement('div');
    footerTitle.className = 'footer-title';
    footerTitle.textContent = 'קישורים נוספים שיעזרו לך';
    footerCard.appendChild(footerTitle);

    // Taaz link
    const taazLink = document.createElement('a');
    taazLink.href = 'https://www.idf.il/%D7%90%D7%AA%D7%A8%D7%99-%D7%99%D7%97%D7%99%D7%93%D7%95%D7%AA/https-wwwidfil-d790d7aad7a8d799d79d-d79ed7a8d79bd796-d7aad7a2d7a1%D7%95%D7%A7%D7%AA-%D7%A2%D7%95%D7%91%D7%93%D7%99-%D7%A6%D7%94%D7%9C/';
    ...
    taazLink.innerHTML = `
        <svg id="Layer_1" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 568.89 544.35" class="taaz-svg">
            <defs>
                <linearGradient id="New_Gradient_Swatch_3" data-name="New Gradient Swatch 3" x1="419.87" y1="414.45" x2="118.1" y2="112.68" gradientUnits="userSpaceOnUse">
                    <stop offset="0" stop-color="#50B8EE"/>
                    <stop offset=".75" stop-color="#0A4A7A"/>
